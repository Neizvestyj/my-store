...
Vue.component('BurgerMenu', {
    data() {
        return {
            //isOpen: false // Переменная для управления состоянием меню
        }
    },
    computed: {
        isOpen() {
    ...
    methods: {
        ...
        onKeydown(e) {
            if (e.key === "Escape" || e.key === "Esc") this.closeMenu();
        },
    },

    mounted() {
        const menu = this.$refs.menuRef;
        this.$store.dispatch('onOutside', menu);
    ...
    //Удаляем обработчкики
    beforeDestroy() {
        this.$store.dispatch("clearOutsideListener")
    },

    template: `
<div class="burger-menu">
<label class="input-title" @click.stop="toggleMenu"
...
<label class="label-svg" @click="toggleMenu">
 <svg class="input-svg" width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
